...
/**
 * Mobile-only variant of routesToFreshnessSegments: react-native-maps has no
 * GPU-vector-tile layer like Mapbox GL, so every Polyline is a discrete
 * native view — rendering one per 8m grid segment (the web app's approach)
 * floods the shadow tree with tens of thousands of views and crashes Yoga's
 * layout engine. This instead emits one feature per whole activity/pass,
 * colored by that activity's own timestamp, trading the web version's
 * per-street-segment overlap resolution for a view count cheap enough for
 * react-native-maps to render.
 */
export function routesToFreshnessRoutes(
  activities: Pick<Activity, "route_points" | "created_at">[],
  now: number = Date.now()
): FeatureCollection<LineString> {
  const features: Feature<LineString>[] = activities
    .filter((activity) => activity.route_points.length >= 2)
    .map((activity, index) => {
      const activityMs = new Date(activity.created_at).getTime();
      const daysAgo = Math.max(0, (now - activityMs) / (24 * 60 * 60 * 1000));
      return {
        type: "Feature",
        id: index,
        geometry: { type: "LineString", coordinates: activity.route_points },
        properties: { daysAgo },
      };
    });
